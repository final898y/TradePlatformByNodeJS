...
function ZodErrorHandling (zoderror:ZodError<ValidateRegister>) :string
{
  ...
  const zodErrorFormatArray = [
    zodErrorFormat.Name?._errors[0],zodErrorFormat.MobilePhone?._errors[0],zodErrorFormat.Email?._errors[0],
    zodErrorFormat.Password?._errors[0],zodErrorFormat.UserStatus?._errors[0],zodErrorFormat.Birthday?._errors[0],
    zodErrorFormat.Address?._errors[0],zodErrorFormat.StoreName?._errors[0]
  ]
